...
export class PDFService {
  ...
  public parse = () => {
    ...
    let docDefinition: any = {
      content: [
        ...
        {
          text:
            "... with a function (opacity at 0 means fully transparent, i.e no color)",
          margin: [0, 20, 0, 8],
        },
        {
          style: "tableExample",
          ...
          layout: {
            fillColor: "blue",
            fillOpacity: function (rowIndex: any, node: any, columnIndex: any) {
    ...
    //console.log(path.join(__dirname, "../img/lv_logo.png"));
    this.logo("lv_logo.png");
    this.h1(this.protocol.metadata.label);
    this.secondary("Metadaten", true);

    this.protocol.entries.map((entry: any) => {
      if (entry.type === "number") {
        this.h2(entry.label + ":");
        entry.value !== null && this.text(entry.value);
      }
    });

    this.doc.moveDown(1.0);
    this.doc.text("Test", { align: "left" });
    */
  ...
  private h1 = (str: string) => {
    this.doc.fontSize(18);
    this.doc.text(str, { align: "center" }).moveDown(1.0);
  };

  private h2 = (str: string, lineBreak?: boolean) => {
    this.bold();
    this.doc.fontSize(13);
    this.doc.text(str, { lineBreak: lineBreak || false });
    this.reset();
  };

  private text = (str: string) => {
    this.doc.fontSize(13);
    this.doc.text(str, { underline: true });
  };

  private secondary = (str: string, center?: boolean) => {
    this.doc.fillColor("gray").fontSize(10);
    this.doc.text(str, { align: center ? "center" : "left" });
    this.reset();
  };

  private logo = (filename: string) => {
    this.doc.image(path.join(__dirname, `../img/${filename}`), 5, 5, {
      width: 50,
    });
  };
